...
$ie.visible=$true
$ie.navigate("https://expertslive.no/sessions/")
...
$Sessions = $ie.Document.getElementsByTagName("li") |Where-Object {$_.className -like "sz-session sz-session--full*"}

$Amount = $Sessions.Count
Write-host "Number of sessions: $Amount"

# Get existing events in calendar for timeframe of conference
$sessionEvents = Find-CalendarEvents -StartDateTime "2019-05-29T06:00:00.000Z" -EndDateTime "2019-05-29T18:00:00.000Z" -pageSize 200
...
foreach ($Session in $Sessions)
{
    ...
    $location = "Room ($room) - Level ($level) - Language ($language) - Speaker ($speaker) - Track ($Tags)"
